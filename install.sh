...
mkdir -p "$MASTODON_PATH/app/javascript/mastodon/features/errordon"
mkdir -p "$MASTODON_PATH/app/javascript/errordon/matrix"
mkdir -p "$MASTODON_PATH/public/matrix"
mkdir -p "$MASTODON_PATH/public/emoji/custom"

echo "🎨 Installing theme SCSS..."
cp "$THEME_DIR/styles/matrix_theme.scss" "$MASTODON_PATH/app/javascript/styles/errordon_matrix.scss"

echo "💻 Installing JavaScript..."
cp "$THEME_DIR/js/matrix_theme.ts" "$MASTODON_PATH/app/javascript/mastodon/features/errordon/"
cp "$THEME_DIR/js/matrix_rain.js" "$MASTODON_PATH/app/javascript/errordon/matrix/index.js"
cp "$THEME_DIR/js/matrix_background.js" "$MASTODON_PATH/app/javascript/errordon/matrix/"
...
cp "$THEME_DIR/rails/initializers/errordon_theme.rb" "$MASTODON_PATH/config/initializers/"
...
echo "   @use 'errordon_matrix';"
echo ""
echo "3. Add to app/javascript/entrypoints/common.ts:"
echo "   import { initMatrixTheme } from 'mastodon/features/errordon/matrix_theme';"
echo "   import '@/errordon/matrix/index.js';"
echo "   initMatrixTheme();"
echo ""
echo "4. Add to .env.production:"
echo "   ERRORDON_MATRIX_THEME_ENABLED=true"
